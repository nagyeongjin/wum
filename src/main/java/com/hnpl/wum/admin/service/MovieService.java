...
public class MovieService {
    ...
    public ScoreDto reviewDetail(Long movieSeq){
        return movieMapper.reviewDetail(movieSeq);
    }

    public void recommend(Long scoreSeq, Long userSeq) {
        ScoreDto dto = reviewDetail(scoreSeq);

        Map<String, Object> map = new HashMap<>();
        map.put("userSeq", userSeq);
        map.put("scoreSeq", scoreSeq);

        Long scoreLikeSeq = movieMapper.userCheck(map);
        int recommend;

        if (scoreLikeSeq != null) {
            recommend = dto.getReviewLike() - 1;
            map.put("likeSeq", scoreLikeSeq);
            movieMapper.removeRecommend(map);
        } else {
            recommend = dto.getReviewLike() + 1;
            movieMapper.insertRecommend(map);
        }

        map.put("reviewLike", recommend);
        movieMapper.changeRecommend(map);
    }
}
